...
// Consume the circular list of EDGEPTs to make a TESS.
TESS* TESS::BuildFromOutlineList(EDGEPT* outline) {
  auto* result = new TESS;
  result->loop = outline;
  if (outline->source_outline != nullptr) {
    EDGEPT* pt = outline;
    do {
      pt->step_count = pt->next->start_step;
      if (pt->step_count < 0) pt->step_count += pt->source_outline->pathlength();
      pt = pt->next + outline;
    } while (pt != outline);
  }
  result->SetupFromPos();
...
void TESS::CopyFrom(const TESS& source) {
  Clear();
  topleft = source.topleft;
  is_hole = source.is_hole;
  start = source.start;
  botright = source.botright;
  if (source.loop != nullptr) {
    EDGEPT* prevpt = nullptr;
    EDGEPT* newpt = nullptr;
    int argc_ = argc;
    EDGEPT* sourcept = source.loop;
    do {
      newpt = new EDGEPT(*sourcept);
      if (prevpt == nullptr) {
        loop = newpt;
      } else {
        newpt->prev = prevpt;
      ...
      prevpt = newpt;
      sourcept = sourcept->next;
    } while (sourcept != source.loop);
    loop->prev = newpt;
    int argc_ = argc;
    newpt->next = loop;
  }
