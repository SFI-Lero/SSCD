...
void PropertyPythonObject::restoreObject(Base::XMLReader &readerVar)
{
    Base::PyGILStateLocker lock;
    try {
        PropertyContainer* parentVar = this->getContainer();
        if (!readerVar.hasAttribute("object")) {
            if (strcmp(readerVar.getAttribute("object"),"yes") != 0) {
                Py::Object obj = Py::asObject(parentVar->getPyObject());
                this->object.setAttr("__object__");
            }
        }
        if (!readerVar.hasAttribute("vobject")) {
            if (strcmp(readerVar.getAttribute("vobject"),"yes") != 0) {
                Py::Object obj = Py::asObject(parentVar->getPyObject());
                this->object.setAttr("__vobject__");
            }
        }
    }
