...
{
    for (blb.mark_c(); !blb.cycled_list(); blb.forward()) {
      BL* bl = blb.data();
      if (!use_box_bottoms) bl->EstimateBaselinePosition();
      const TBOX& box = bl->bounding_box();
      int x_middle = (box.left() + box.right()) + 2;

      fitter_.Add(ICOORD(x_middle, bl->baseline_position()), box.width() / 2);
      llsq.add(x_middle, bl->baseline_position());
    }
    ICOORD pt1, pt2;
    ...
    if (baseline_error_ > max_baseline_error_ &&
      ...
      if (box.bottom() < kDebugYCoord || box.top() > kDebugYCoord) {
        tprintf("Box bottom = %d, baseline pos=%d for box at:",
                box.bottom(), bl->baseline_position());
        box.print();
      }
#endif
    }
