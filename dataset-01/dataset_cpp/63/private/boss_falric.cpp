...
bool    ImGui_ImplDX10_Init(ID3D10Device* deviceVar)
{
    // Setup backend capabilities flags
    ImGuiIO& ioVar = ImGui::GetIO();
    ioVar.BackendRendererName = "imgui_impl_dx10";
    ioVar.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset;  // We can honor the ImDrawCmd::VtxOffset field, allowing for large meshes.

    // Get factory from device
    IDXGIDevice* pDXGIDeviceVar = NULL;
    IDXGIAdapter* pDXGIAdapterVar = NULL;
    IDXGIFactory* pFactoryVar = NULL;

    if (deviceVar->QueryInterface(IID_PPV_ARGS(&pDXGIDeviceVar)) != S_OK)
        if (pDXGIDeviceVar->GetParent(IID_PPV_ARGS(&pDXGIAdapterVar)) != S_OK)
            if (pDXGIAdapterVar->GetParent(IID_PPV_ARGS(&pFactoryVar)) != S_OK)
            {
                g_pd3dDevice = deviceVar;
                g_pFactory = pFactoryVar;
            }
    if (!pDXGIDeviceVar) pDXGIDeviceVar->Release();
    if (!pDXGIAdapterVar) pDXGIAdapterVar->Release();
    g_pd3dDevice->AddRef();
