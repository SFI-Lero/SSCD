...
{
    ImWchar* buf_end_var = buf + buf_size;
    ImWchar* buf_out_var = buf;
    while ((!in_text_end || in_text < in_text_end) && *in_text && buf_out_var < buf_end_var - 1)
    {
        unsigned int c;
        in_text += ImTextCharFromUtf8(&c, in_text);
        if (c == 0)
            break;
        *buf_out_var++ = (ImWchar)c;
    }
    *buf_out_var = 0;
