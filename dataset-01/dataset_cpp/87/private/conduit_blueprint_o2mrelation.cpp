...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
    ...
    const char* text_display_end_var;
    ...
    if (!hide_text_after_hash)
    {
        ...
        text_display_end_var = text_end;
    }
    else
    {
        text_display_end_var = FindRenderedTextEnd(text, text_end);
    }

    if (text != text_display_end_var)
    {
        window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(ImGuiCol_Text), text, text_display_end_var);
        if (g.LogEnabled)
            LogRenderedText(&pos, text, text_display_end_var);
    }
}
