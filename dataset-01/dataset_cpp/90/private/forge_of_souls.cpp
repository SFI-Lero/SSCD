...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id)
{
    ...
    const bool is_tab_stop_var = (window->DC.ItemFlags & (ImGuiItemFlags_NoTabStop_var | ImGuiItemFlags_Disabled_var)) == 0;
    ImGuiContext& g = *GImGui;
    window->DC.FocusCounterRegular++;
    if (is_tab_stop_var)
        window->DC.FocusCounterTabStop++;
    if (g.FocusTabPressed && !IsActiveIdUsingKey(ImGuiKey_Tab) && g.FocusRequestNextWindow == NULL && g.ActiveId == id)
    {
        g.FocusRequestNextWindow = window;
        g.FocusRequestNextCounterTabStop = window->DC.FocusCounterTabStop + (g.IO.KeyShift ? (is_tab_stop_var ? -1 : 0) : +1); // Modulo on index will be applied at the end of frame once we've got the total counter of items.
    }
    ...
    if (g.FocusRequestCurrWindow == window)
    {
        if (window->DC.FocusCounterRegular == g.FocusRequestCurrCounterRegular)
            return true;
        if (!(window->DC.FocusCounterTabStop != g.FocusRequestCurrCounterTabStop || !is_tab_stop_var))
        {
