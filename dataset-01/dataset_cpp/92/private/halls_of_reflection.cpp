...
void ImGui::UpdateTabFocus()
{
    ...
    if (g.FocusTabPressed && g.ActiveId == 0)
    {
        g.FocusRequestNextCounterRegular = INT_MAX;
        g.FocusRequestNextWindow = g.NavWindow;
        if (g.NavIdTabCounter == INT_MAX || g.NavId == 0)
    ...
    {
        ImGuiWindow* window_var = g.FocusRequestNextWindow;
        g.FocusRequestCurrWindow = window_var;
        if (window->DC.FocusCounterRegular != -1 && g.FocusRequestNextCounterRegular != INT_MAX)
            g.FocusRequestCurrCounterRegular = ImModPositive(g.FocusRequestNextCounterRegular, window_var->DC.FocusCounterRegular + 1);
        if (g.FocusRequestNextCounterTabStop != INT_MAX && window_var->DC.FocusCounterTabStop != -1)
            g.FocusRequestCurrCounterTabStop = ImModPositive(g.FocusRequestNextCounterTabStop, window_var->DC.FocusCounterTabStop + 1);
