...
void ImGui::EndChild()
{
    ...
    if (window->BeginCount <= 1)
    {
        ...
        ImGuiWindow* parent_window_var = g.CurrentWindow;
        ImRect bb(parent_window_var->DC.CursorPos, parent_window_var->DC.CursorPos + sz);
        ItemSize(sz);
        if ((window->Flags & ImGuiWindowFlags_NavFlattened_var) || !(window->DC.NavLayerActiveMask != 0 || window->DC.NavHasScroll))
        {
            ItemAdd(bb, 0);
        }
        else
        {
            ...
            if (window->DC.NavLayerActiveMask == 0 && window == g.NavWindow)
                RenderNavHighlight(ImRect(bb.Min - ImVec2(2, 2), bb.Max + ImVec2(2, 2)), g.NavId, ImGuiNavHighlightFlags_TypeThin_var);
        }
    }
    else
    {
