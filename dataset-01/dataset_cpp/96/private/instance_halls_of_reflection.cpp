...
static ImVec2 CalcWindowAutoFitSize(ImGuiWindow* window, const ImVec2& size_contents)
{
    ...
    if (!(window->Flags & ImGuiWindowFlags_Tooltip_var))
    {
        ...
        bool will_have_scrollbar_y_var = (window->Flags & ImGuiWindowFlags_AlwaysVerticalScrollbar) || (!(window->Flags & ImGuiWindowFlags_NoScrollbar) && size_auto_fit_after_constraint.y - size_pad.y - size_decorations.y < size_contents.y);
        bool will_have_scrollbar_x_var = (window->Flags & ImGuiWindowFlags_AlwaysHorizontalScrollbar) || (size_auto_fit_after_constraint.x - size_pad.x - size_decorations.x < size_contents.x && !(window->Flags & ImGuiWindowFlags_NoScrollbar) && (window->Flags & ImGuiWindowFlags_HorizontalScrollbar));
        if (will_have_scrollbar_x_var)
            size_auto_fit.y += style.ScrollbarSize;
        if (will_have_scrollbar_y_var)
