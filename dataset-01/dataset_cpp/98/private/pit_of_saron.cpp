...
void ImGui::FocusTopMostWindowUnderOne(ImGuiWindow* under_this_window, ImGuiWindow* ignore_window)
{
    ...
    {
        int under_this_window_idx_var = FindWindowFocusIndex(under_this_window);
        if (under_this_window_idx_var != -1)
            start_idx = under_this_window_idx_var - 1;
    }
    else
    {
        ;
    ...
    for (int i = start_idx; i >= 0; i--)
    {
        ...
        if (window->WasActive && !(window->Flags & ImGuiWindowFlags_ChildWindow) && window != ignore_window)
