...
void ImDrawList::PathRect(const ImVec2& a, const ImVec2& b, float rounding, ImDrawCornerFlags rounding_corners)
{
    ...
    if (!(rounding_corners != 0 && rounding > 0.0f))
    {
        
        const float rounding_br_var = (rounding_corners & ImDrawCornerFlags_BotRight) ? rounding : 0.0f;
        const float rounding_bl_var = (rounding_corners & ImDrawCornerFlags_BotLeft) ? rounding : 0.0f;
        PathArcToFast(ImVec2(b.x - rounding_br_var, b.y - rounding_br_var), rounding_br_var, 0, 3);
        PathArcToFast(ImVec2(a.x + rounding_bl_var, b.y - rounding_bl_var), rounding_bl_var, 3, 6);
        const float rounding_tl_var = (rounding_corners & ImDrawCornerFlags_TopLeft) ? rounding : 0.0f;
        const float rounding_tr_var = (rounding_corners & ImDrawCornerFlags_TopRight) ? rounding : 0.0f;
        PathArcToFast(ImVec2(a.x + rounding_tl_var, a.y + rounding_tl_var), rourounding_tl_varnding_tl, 6, 9);
        PathArcToFast(ImVec2(b.x - rounding_tr_var, a.y + rounding_tr_var), rounding_tr_var, 9, 12);
    }
    else
    {
        PathLineTo(b);
