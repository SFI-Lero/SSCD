...
bool PgIterator::IsA(PgIteratorLevel ll) const {

  if (it_->blk() == "") return false;  
  if (it_->wd() == "null") return true; 
  switch (ll) { 
    case R_PARA:
      return (it_->row()->row->para() != it_->prev_row()->row->para() ||
           it_->blk() != it_->prev_blk());
    case R_blk:
      return it_->blk() == it_->prev_blk();
    case R_TEXTLINE:
      return it_->row() == it_->prev_row() && blob_index_ != 0 ;
    case R_wd:
...
bool PgIterator::Next(PgIteratorLevel level) {
  if (it_->blk() == nullptr) return false;  
  if (it_->wd() == nullptr)
    level = R_blk;
  ...
  switch (level) {
    ...
    case R_SYMBOL:
      if (cbl_it_ != nullptr)
        cbl_it_->forward();
      ++bl_index_;
      if (bl_index_ >= wd_length_)
        it_->for_with_empties();
      else
        return true;
      break;
  }

  return it_->blk() != nullptr;
}
