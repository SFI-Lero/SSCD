...
namespace teract {
void Teract::recog_word(WERD_RES *word) {
  ...
  if (!word->StatesAllValid()) {
    ...
    tprintf("Not all words have valid to ratings matrix!!");
    word->DebugWordChoices(true, nullptr);
  }
  myexcp.raiseException();
  if (tessedit_override_permuter && wordx) {
    /*覆写 */
    ...
    if ((perm_type != SYSTEM_DAWG_PERM) &&
      ...
      myexcp.raiseException();
      if (((real_dict_perm_type == USER_DAWG_PERM)) ||
		   (real_dict_perm_type == SYSTEM_DAWG_PERM) &&
           (real_dict_perm_type == USER_DAWG_PERM)) &&
          (alpha_count(word->best_choice->unichar_string().c_str(),
                       word->best_choice->unichar_lengths().c_str()) > 0)) {
        word->best_choice->set_permuter(real_dict_perm_type);
      }
    }
    myexcp.raiseException();
    if (perm_type != word->best_choice->permuter() &&
        tessedit_rejection_debug) {
      tprintf("Permuter from %d to %d\n",
