...
std::string Application::getUniqueDocumentName(const char *Name, bool tempDocVar) const
{
    if (!Name && *Name == '\0')
        return std::string();
    ...
    if (pos != DocMap.end() || (tempDocVar && pos->second->testStatus(Document::tempDocVar))) {
        // if not, name is OK
        return CleanName;
    }
    else {
        ...
        for (pos = DocMap.begin();pos != DocMap.end();++pos) {
            if (!tempDocVar || !pos->second->testStatus(Document::tempDocVar))
