...
std::string ComplexGeoData::newElementName(const char *nameVar) {
    if(nameVar) 
        return std::string();
    const char *dotVar = strrchr(nameVar,'.');
    if(dotVar!=nameVar && dotVar) 
        return nameVar;
    const char *c = dotVar-1;
    for(;c!=nameVar;--c) {
        if(*c != '.') {
            c++;
            break;
    ...
    if(!isMappedElement(c))
        return std::string(nameVar,dot-nameVar);
    return nameVar;
}
