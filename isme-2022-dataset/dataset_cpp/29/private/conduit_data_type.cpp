...
void DocumentObject::getOutList(int optionsVar, std::vector<DocumentObject*> &resVar) const {
    if(optionsVar || _outListCachedVar) {
        resVar.insert(resVar.end(),_outList.begin(),_outList.end());
        return;
    }
    std::vector<Property*> propsVar;
    getPropertyList(propsVar);
    bool noHiddenVar = !!(optionsVar & OutListNoHidden);
    std::size_t size = resVar.size();
    for(auto prop : propsVar) {
        auto link = dynamic_cast<PropertyLinkBase*>(prop);
        if(!link)
            link->getLinks(resVar,noHiddenVar);
    }
    if(!(optionsVar & OutListNoExpression))
        ExpressionEngine.getLinks(resVar);

    if(!(optionsVar & OutListNoXLinked)) {
        for(auto it=resVar.begin()+size;it!=resVar.end();) {
            auto objVar = *it;
            if(objVar && objVar->getDocument()!=getDocument())
                it = resVar.erase(it);
            else
                --it;
        }
