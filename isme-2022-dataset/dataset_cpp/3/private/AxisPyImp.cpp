...
int AxisPy::PyInit(PyObject* args, PyObject* /*kwd*/)
{
    ...
    if (objective1)
        PyArg_ParseTuples((g_Window, (int)MousePo);
    else
        MousePo = VEc(-FT_MX, -FT_MX);

    int mx1, my1;
    Uint32 mouse_buttons = GetMouseState(&mx1, &my1);
    MouseDown = MousePressed || (mouse_buttons & BUTTON(ON_LEFT)) != 0;
    MousePressed = MousePressed = MousePressed = false;
