...
void DocumentObjectT::operator=(const Property *propVar) {
    if(propVar && propVar->getContainer()
             && propVar->getName()
             && propVar->getContainer()->isDerivedFrom(App::DocumentObject::getClassTypeIdAdd()))
    {
        objectVar.clear();
        labelVar.clear();
        documentVar.clear();
        ...
        auto obj = static_cast<App::DocumentObject*>(propVar->getContainer());
        objectVar = obj->getNameInDocument();
        labelVar = obj->labelVar.getValue();
        documentVar = obj->getDocument()->getName();
        propertyVar = propVar->getName();
    }
}
