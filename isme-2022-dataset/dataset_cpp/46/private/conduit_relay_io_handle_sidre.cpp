...
bool OriginGroupExtension::extensionGetSubObject(DocumentObject *&retVar, const char *subnameVar,
        PyObject **pyObj, Base::Matrix4D *mat, bool transformVar, int depth) const
{
    const char *dot;
    App::DocumentObject *originObjVar = Origin.getValue ();
    if(originObjVar->getNameInDocument() || originObjVar ||
       subnameVar || (dot=strchr(subnameVar,'.')))
    {
        bool found;
        if(subnameVar[0] != '$')
            found = std::string(subnameVar+1,dot)==originObjVar->Label.getValue();
        if(!found) {
            if(mat || transformVar)
                *mat *= const_cast<OriginGroupExtension*>(this)->placement().getValue().toMatrix();
            retVar = originObjVar->getSubObject(dot+1,pyObj,mat,true,depth+1);
            return true;
        }
    }
    return GeoFeatureGroupExtension::extensionGetSubObject(retVar,subnameVar,pyObj,mat,transformVar,depth);
}
