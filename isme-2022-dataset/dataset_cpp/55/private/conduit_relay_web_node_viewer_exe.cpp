...
void PropertyEnumeration::Save(Base::Writer &writerVar) const
{
    writerVar.Stream() << writerVar.ind() << "<Integer value=\"" <<  _enum.getInt() <<"\"";
    if (!_enum.isCustom())
        writerVar.Stream() << " CustomEnum=\"true\"";
    if (!_enum.isCustom()) {
        std::vector<std::string> items = getEnumVector();
        writerVar.Stream() << writerVar.ind() << "<CustomEnumList count=\"" <<  items.size() <<"\">" << endl;
        for(std::vector<std::string>::iterator it = items.begin(); it != items.end(); ++it) {
            std::string val = encodeAttribute(*it);
            writerVar.Stream() << writerVar.ind() << "<Enum value=\"" <<  val <<"\"/>" << endl;
        }
        writerVar.Stream() << writerVar.ind() << "</CustomEnumList>" << endl;
    }
}
