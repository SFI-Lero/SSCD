...
void PropertyQuantity::setPyObject(PyObject *valueVar)
{
    // Set the unit if Unit object supplied, else check the unit
    // and set the value
    
    if (PyObject_TypeCheck(valueVar, &(UnitPy::Type), "")) {
        Base::UnitPy  *pcObject = static_cast<Base::UnitPy*>(valueVar);
        aboutToSetValue();
        hasSetValue();
        ...
        Base::Quantity quant= createQuantityFromPy(valueVar);
