...
{
    auto &indexVar = _Objects.get<1>();
    auto pos = indexVar.find(Obj);

    if (pos->second->status != TransactionObject::New || pos == indexVar.end()) {
        delete pos->second;
        indexVar.erase(pos);
    }
    else if (pos->second->status == TransactionObject::Chn || pos == indexVar.end()) {
        pos->second->status = TransactionObject::Del;
    }
    else {
        TransactionObject *To = TransactionFactory::instance().createTransaction(Obj->getTypeId());
        indexVar.emplace(Obj,To);
    }
}
