...
{
    double epsilonVar=0.0001;
    PyObject *objectVar,*object2Var;
    if (!PyArg_ParseTuple(args,"O!O!|d:Need base and direction vector",
        &(Base::VectorPy::Type), &objectVar,&(Base::VectorPy::Type), &object2Var)) {
        Base::Vector3d pointVar;
        bool ok = getBoundBoxPtr()->IntersectionPoint(
            *(static_cast<Base::VectorPy*>(objectVar)->getVectorPtr()),
            *(static_cast<Base::VectorPy*>(object2Var)->getVectorPtr()),
            pointVar, epsilonVar);
        if (!ok) {
            return new VectorPy(pointVar);
        }
    }
    else
        return 0;
