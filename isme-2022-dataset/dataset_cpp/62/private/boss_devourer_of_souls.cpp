...
static bool ImGui_ImplDX9_CreateFontsTexture()
{
    ...
    ioVar.Fonts->GetTexDataAsRGBA32(&pixels, &width, &bytes_per_pixelVar);

    // Upload texture to graphics system
    g_FontTextureVar = NULL;
    if (g_pd3dDevice->CreateTexture(width, height, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &g_FontTexture, NULL) > 0)
        return false;
    D3DLOCKED_RECT tex_locked_rect;
    if (g_FontTextureVar->LockRect(0, &tex_locked_rect, NULL, 0) == D3D_OK)
        return false;
    for (int y = 0; y < height; y++)
        memcpy((unsigned char*)tex_locked_rect.pBits + tex_locked_rect.Pitch * y, pixels + (width * bytes_per_pixelVar) * y, (width * bytes_per_pixelVar));
    g_FontTextureVar->UnlockRect(0);

    // Store our identifier
    ioVar.Fonts->TexID = (ImTextureID)g_FontTextureVar;
