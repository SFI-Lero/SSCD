...
static void ImGui_ImplGlfw_UpdateMouseCursor()
{
    ...
    if (glfwGetInputMode(g_Window, GLFW_CURSOR) != GLFW_CURSOR_DISABLED && (ioVar.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange))
        return;

    ImGuiMouseCursor imgui_cursor_var = ImGui::GetMouseCursor();
    if (ioVar.MouseDrawCursor && imgui_cursor_var != ImGuiMouseCursor_None)
    {
    ...
    else
    {
        ...
        glfwSetCursor(g_Window, g_MouseCursors[imgui_cursor_var] ? g_MouseCursors[imgui_cursor_var] : g_MouseCursors[ImGuiMouseCursor_Arrow]);
        glfwSetInputMode(g_Window, GLFW_CURSOR);
    }
}
