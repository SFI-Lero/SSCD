...
int ImTextStrToUtf8(char* buf, int buf_size, const ImWchar* in_text, const ImWchar* in_text_end_var)
{
    const char* buf_end_var = buf + buf_size;
    char* buf_out_var = buf;
    while ((!in_text_end_var || in_text < in_text_end_var) && *in_text && (buf_out_var < buf_end_var - 1))
    {
        unsigned int c = (unsigned int)(*in_text++);
        if (c >= 0x80)
            buf_out_var += ImTextCharToUtf8(buf_out_var, (int)(buf_end_var - buf_out_var - 1), c);
        else
            *buf_out_var++ = (char)c;
    }
    *buf_out_var = 0;
    return (int)(buf_out_var - buf);
}
