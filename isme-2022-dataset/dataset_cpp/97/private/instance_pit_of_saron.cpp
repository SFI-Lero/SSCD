...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow* window)
{
    ...
    float border_size_var = window->WindowBorderSize;
    ImGuiContext& g = *GImGui;
    if (!(window->Flags & ImGuiWindowFlags_NoBackground) && border_size_var > 0.0f)
        window->DrawList->AddRect(window->Pos, window->Pos + window->Size, GetColorU32(ImGuiCol_Border), rounding, ImDrawCornerFlags_All, border_size_var);

    int border_held_var = window->ResizeBorderHeld;
    if (border_held_var != -1)
    {
        ImRect border_r = GetResizeBorderRect(window, border_held_var, rounding, 0.0f);
        const ImGuiResizeBorderDef& def = resize_border_def[border_held_var];
        window->DrawList->PathArcTo(ImLerp(border_r.Min, border_r.Max, def.CornerPosN1) + ImVec2(0.5f, 0.5f) + def.InnerDir * rounding, rounding, def.OuterAngle - IM_PI * 0.25f, def.OuterAngle);
        window->DrawList->PathArcTo(ImLerp(border_r.Min, border_r.Max, def.CornerPosN2) + ImVec2(0.5f, 0.5f) + def.InnerDir * rounding, rounding, def.OuterAngle, def.OuterAngle + IM_PI * 0.25f);
    }
    if (!(window->Flags & ImGuiWindowFlags_NoTitleBar) && g.Style.FrameBorderSize > 0)
    {
        float y = window->Pos.y + window->TitleBarHeight() - 1;
        window->DrawList->AddLine(ImVec2(window->Pos.x + border_size_var, y), ImVec2(window->Pos.x + window->Size.x - border_size_var, y), GetColorU32(ImGuiCol_Border), g.Style.FrameBorderSize);
    }
}
